...
function resolveSessionKey(input: {
  ...
  const baseKey = input.strategy === "run" 
    ? `paperclip:run:${input.runId}`
    : input.strategy === "issue" && input.issueId 
      ? `paperclip:issue:${input.issueId}` 
      : fallback;
  return input.agentId ? `agent:${input.agentId}:${baseKey}` : baseKey;
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const payloadAgentId = nonEmpty(payloadTemplate.agentId);
  const effectiveAgentId = configuredAgentId ?? payloadAgentId ?? null;
